...
public:
    // Default constructor
    Question() : id(0), ques_text(""), answer("empty") {}
    
    // Custom constructor
    Question(int q_id, const string& text) : id(q_id), ques_text(text), answer("empty") {}
    
    void answer_question(const string& ans) {
        answer = ans;
    }

    void print() const {
        cout << "Question ID: " << id << ", Text: " << ques_text << ", Answer: " << answer << endl;
    }
};
